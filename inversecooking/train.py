...
from inversecooking.data_loader import get_loader
from inversecooking.model import get_model
from inversecooking.model import mask_from_eos, label2onehot
from inversecooking.utils.metrics import softIoU, compute_metrics, update_error_types
from inversecooking.utils.tb_visualizer import Visualizer
                ...
                if args.log_step != -1 and i % args.log_step == 0:
                    ...
                    if args.tensorboard:
                        ...
                        logger.scalar_summary(mode=split + '_iter', epoch=total_step * epoch + i,
                                              **{k: np.mean(v[-args.log_step:]) for k, v in total_loss_dict.items() if
                                                 v})
